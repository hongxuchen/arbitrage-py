...
class ArbitrageMediator(QtCore.QThread):
    _logger = common.setup_logger()

    def __init__(self, api, parent=None):
        super(ArbitrageMediator, self).__init__(parent)
        self.api = api
        self.pending = True

    def _get_order_info(self, order_id):
        order_info = self.api.order_info(order_id)
        ArbitrageMediator._logger.debug('ORDER_INFO:{}'.format(order_info))
        return order_info
    ...
    def adjust_price(self, order_id):
        ...
        if order_info.seconds_since_created() > config.max_pending_seconds:
            ...
            # discard, simply cancel
                ...
                return
            ArbitrageMediator._logger.debug("adjust_price")
    ...
    def run(self):
        ...
        # while self.pending:
        #     self.adjust_price()
        # ArbitrageMediator._logger.debug('finished inside')
...
if __name__ == '__main__':
    ...
    monitor = ArbitrageMediator(api, order_id)
    monitor.start()
    monitor.wait()
    print('\nanother......')
